...
GtkWidget *label_ficha;
GtkWidget *label_modo;
GtkWidget *label_jugador1;
GtkWidget *label_jugador2;
...
GtkWidget *imagenes[9][9];

GtkWidget *btn_pcvspc;
GtkWidget *btn_pcvsh;
GtkWidget *btn_aleatoriomodo;
GtkWidget *btn_colorblanca;
GtkWidget *btn_fichaNegra;
GtkWidget *btn_aleatorioFicha;
...
void funcbtn_modo_pcvspc(GtkWidget *widget, gpointer data)
{
	gchar *temp = g_strdup_printf("Has elegido que juegue PC vs PC");
	gtk_label_set_text(GTK_LABEL(label_modo), temp);
	g_free(temp);
}
void funcbtn_modo_pcvshumano(GtkWidget *widget, gpointer data)
{
	gchar *temp = g_strdup_printf("Has elegido que juegue PC vs HUMANO");
	gtk_label_set_text(GTK_LABEL(label_modo), temp);
	g_free(temp);
}
void funcbtn_modo_random(GtkWidget *widget, gpointer data)
{
	srand(time(NULL));
	modo_juego = rand() % 2;
	if(modo_juego){
...
void funcbtn_modo_blanca(GtkWidget *widget, gpointer data)
{
	gchar *temp = g_strdup_printf("Jugaras con las fichas Blancas");
	gtk_label_set_text(GTK_LABEL(label_ficha), temp);
	g_free(temp);
}

void funcbtn_modo_negra(GtkWidget *widget, gpointer data)
{
	gchar *temp = g_strdup_printf("Jugaras con las fichas Negras");
	gtk_label_set_text(GTK_LABEL(label_ficha), temp);
	g_free(temp);
}
void funcbtn_modo_aleatorio(GtkWidget *widget, gpointer data)
{
	srand(time(NULL));
	color_ficha = rand() % 2;
	if(color_ficha){
...
{
	nombre_usuario= gtk_entry_get_text (GTK_ENTRY (entry_jugador1));
	gtk_label_set_text(GTK_LABEL(label_jugador1), nombre_usuario);

	nombre_rival= gtk_entry_get_text (GTK_ENTRY (entry_jugador2));
	gtk_label_set_text(GTK_LABEL(label_jugador2), nombre_rival);
...
void tablero_cb(GtkWidget *event_box, GdkEventButton *event, gpointer data){
	...
	if(bandera ==0){
		...
		if (tablerito[x][y] == 'N'){
			...
			bandera =1;
			//gtk_image_set_from_file(GTK_IMAGE(imagenes[3][3] ),"./IMG/blanca_claro.png");

			g_print("./IMG/Ficha_Negra.png");

		}
		else{
			g_print("./IMG/Ficha_Blanca.png");
			//g_print("%s",tablerito[x][y]);
		}


	}
	else{
		x2 = (GUINT_FROM_LE(event->y) / 50); //las imagenes tienen son 50x50pixeles
		y2 = (GUINT_FROM_LE(event->x) / 50);
		gtk_image_set_from_file(GTK_IMAGE(imagenes[x2][y2] ),"./IMG/Ficha_Negra.png");
		gtk_image_set_from_file(GTK_IMAGE(imagenes[x][y] ),"./IMG/cruz.png");
		tablerito[x][y] = 'C';
		tablerito[x2][y2] = 'N';
		bandera =0;
	}
...
GtkWidget *crear_tablero(){
	...
	char *fichas[3] = {"./IMG/Ficha_Negra.png","./IMG/Ficha_Blanca.png","./IMG/cruz.png"};
	char *lados[4]  = {"./IMG/diagonalDerecha.png","./IMG/diagonalIzquierda.png","./IMG/horizontal.png","./IMG/vertical.png"};
	...
	for (i = 0; i < MAX_FILA; i++)
	{
		for (j = 0; j < MAX_COLUMNA; j++) {
			...
			else if((i + j) % 2 == 0)
			{
				...
				{
					tablerito[i][j] = 'P';
					imagenes[i][j] = gtk_image_new_from_file(lados[0]);
					gtk_grid_attach(GTK_GRID(GTKtablero), GTK_WIDGET(imagenes[i][j]), j, i, 1, 1);
				}
				else
				{
					tablerito[i][j] = 'S';
					imagenes[i][j] = gtk_image_new_from_file(lados[1]);
					gtk_grid_attach(GTK_GRID(GTKtablero), GTK_WIDGET(imagenes[i][j]), j, i, 1, 1);
				}
			}
			else if (i % 2 == 0)
			{
				tablerito[i][j] = 'H';
				imagenes[i][j] = gtk_image_new_from_file(lados[2]);
				gtk_grid_attach(GTK_GRID(GTKtablero), GTK_WIDGET(imagenes[i][j]), j, i, 1, 1);
			}
			else if (j % 2 == 0)
			{
				tablerito[i][j] = 'V';
				imagenes[i][j] = gtk_image_new_from_file(lados[3]);
				gtk_grid_attach(GTK_GRID(GTKtablero), GTK_WIDGET(imagenes[i][j]), j, i, 1, 1);
			}
...
void realizar_jugada(Jugada j)
{
 ...
 int main (int argc, char *argv[])
 {
	...
	btn_pcvsh = GTK_WIDGET(gtk_builder_get_object(builder, "btn_pcvsh"));
	btn_aleatoriomodo = GTK_WIDGET(gtk_builder_get_object(builder, "btn_aleatoriomodo"));
	btn_colorblanca = GTK_WIDGET(gtk_builder_get_object(builder, "btn_colorblanca"));
	btn_fichaNegra = GTK_WIDGET(gtk_builder_get_object(builder, "btn_fichaNegra"));
	btn_aleatorioFicha = GTK_WIDGET(gtk_builder_get_object(builder, "btn_aleatorioFicha"));
	...
	label_modo = GTK_WIDGET(gtk_builder_get_object(builder, "label_modo"));
	label_ficha = GTK_WIDGET(gtk_builder_get_object(builder, "label_ficha"));
	label_jugador1 = GTK_WIDGET(gtk_builder_get_object(builder, "label_jugador1"));
	label_jugador2 = GTK_WIDGET(gtk_builder_get_object(builder, "label_jugador2"));
	...
	g_signal_connect(btn_pcvspc, "button-press-event", G_CALLBACK(funcbtn_modo_pcvspc), NULL);
	g_signal_connect(btn_pcvsh, "button-press-event", G_CALLBACK(funcbtn_modo_pcvshumano), NULL);
	g_signal_connect(btn_aleatoriomodo, "button-press-event", G_CALLBACK(funcbtn_modo_random), NULL);
	g_signal_connect(btn_colorblanca, "button-press-event", G_CALLBACK(funcbtn_modo_blanca), NULL);
	g_signal_connect(btn_fichaNegra, "button-press-event", G_CALLBACK(funcbtn_modo_negra), NULL);
	g_signal_connect(btn_aleatorioFicha, "button-press-event", G_CALLBACK(funcbtn_modo_aleatorio), NULL);
